...
/**
 * Check if php files contains laravel specific dd statement
 */
class Dd extends RunnersAbstract
{
    protected $_name = "dd";
    ...
    function run(array $files)
    {
        foreach ($files as $file) {
            //check if files have dd statement
            //but dont check files which are commented with //
            $out = Sys::runCommand(
                $this->_vcs->catCommand($file) .
                " | grep -vE '^\s*\/\/' | grep -iqE 'dd' 2>&1"
            );

            if ($out['exitCode'] == 0) {
                $out['output'][] = "'${file}' file contains dd statement";
